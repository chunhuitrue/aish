...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Aish:";
...
pub enum Op {
    ...
    /// for a turn of a `AishConversation`.
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// Skills are loaded globally from ~/.aish/skills (user), system cache, and admin paths.
    ListSkills {
...
pub enum SandboxPolicy {
    ...
    /// working directory.
    #[serde(rename = "current-dir-write")]
    CurrentDirWrite {
...
/// privileges of the agent (e.g. `.aish`, `.git`, notably `.git/hooks`) under
...
impl SandboxPolicy {
    ...
    pub fn new_current_dir_write_policy() -> Self {
    ...
    pub fn has_full_disk_write_access(&self) -> bool {
        match self {
            ...
            SandboxPolicy::CurrentDirWrite { .. } => false,
        }
    }
    ...
    pub fn has_full_network_access(&self) -> bool {
        match self {
            ...
            SandboxPolicy::ReadOnly => false,
            SandboxPolicy::CurrentDirWrite { network_access, .. } => *network_access,
        }
    }

    /// Returns the list of writable roots together with subpaths that should
    /// remain read‑only under each writable root.
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        match self {
            ...
            SandboxPolicy::CurrentDirWrite {
                ...
                // Start with cwd as the primary writable root.
                let mut roots: Vec<AbsolutePathBuf> = Vec::new();
                if let Ok(cwd_abs) = AbsolutePathBuf::from_absolute_path(cwd) {
                    roots.push(cwd_abs);
                }

                // Include /dev/shm on Unix for IPC (shared memory, semaphores).
                // This is needed for multiprocessing locks and similar synchronization primitives.
                if cfg!(unix) {
                    #[allow(clippy::expect_used)]
                    let dev_shm = AbsolutePathBuf::from_absolute_path("/dev/shm")
                        .expect("/dev/shm is absolute");
                    if dev_shm.as_path().is_dir() {
                        roots.push(dev_shm);
                    }
                }
                ...
                roots
                    ...
                    .map(|writable_root| {
                        ...
                        let top_level_codex = writable_root
                            .join(".aish")
                            .expect(".aish is a valid relative path");
...
/// Aish errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum AishErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub codex_error_info: Option<AishErrorInfo>,
}
...
#[derive(
    Debug, Clone, Copy, Display, Default, Deserialize, Serialize, PartialEq, Eq, JsonSchema, TS,
)]
#[serde(rename_all = "snake_case")]
...
pub struct SessionConfiguredEvent {
    ...
    /// The current working directory where commands will be executed.
    pub cwd: PathBuf,
    ...
    fn serialize_event() -> Result<()> {
        ...
        let event = Event {
            ...
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                ...
                model: "aish-mini-latest".to_string(),
