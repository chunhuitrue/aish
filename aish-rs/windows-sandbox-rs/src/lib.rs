...
#[cfg(target_os = "windows")]
pub use token::create_current_dir_write_token_with_cap_from;
#[cfg(target_os = "windows")]
...
mod windows_impl {
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let is_current_dir_write = matches!(&policy, SandboxPolicy::CurrentDirWrite { .. });
        ...
        let (h_token, psid_to_use): (HANDLE, *mut c_void) = unsafe {
            match &policy {
                ...
                SandboxPolicy::CurrentDirWrite { .. } => {
                    ...
                    super::token::create_current_dir_write_token_with_cap(psid)?
                }
                SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        let persist_aces = is_current_dir_write;
    ...
    mod tests {
        ...
        fn applies_network_block_when_access_is_disabled() {
            assert!(should_apply_network_block(&current_dir_policy(false)));
        }

        #[test]
        fn skips_network_block_when_access_is_allowed() {
            assert!(!should_apply_network_block(&current_dir_policy(true)));
        }
