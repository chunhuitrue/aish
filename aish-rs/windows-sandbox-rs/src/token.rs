...
/// # Safety
/// Caller must close the returned token handle.
pub unsafe fn create_current_dir_write_token_with_cap(
    psid_capability: *mut c_void,
) -> Result<(HANDLE, *mut c_void)> {
    let base = get_current_token_for_restriction()?;
    let res = create_current_dir_write_token_with_cap_from(base, psid_capability);
    CloseHandle(base);
    res
}
...
pub unsafe fn create_current_dir_write_token_with_cap_from(
    base_token: HANDLE,
    psid_capability: *mut c_void,
) -> Result<(HANDLE, *mut c_void)> {
