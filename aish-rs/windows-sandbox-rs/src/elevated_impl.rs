mod windows_impl {
    ...
    /// Locates `aish-command-runner.exe` next to the current binary.
    fn find_runner_exe() -> PathBuf {
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                let candidate = dir.join("aish-command-runner.exe");
                if candidate.exists() {
                    return candidate;
                }
            }
        }
        PathBuf::from("aish-command-runner.exe")
    }

    /// Generates a unique named-pipe path used to communicate with the runner process.
    fn pipe_name(suffix: &str) -> String {
        let mut rng = SmallRng::from_entropy();
        format!(r"\\.\pipe\aish-runner-{:x}-{}", rng.gen::<u128>(), suffix)
    }
    ...
    struct RunnerPayload {
        ...
        // Writable log dir for sandbox user (.aish in sandbox profile).
        codex_home: PathBuf,
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let runner_cmdline = runner_exe
            .to_str()
            .map(|s| s.to_string())
            .unwrap_or_else(|| "aish-command-runner.exe".to_string());
    ...
    mod tests {
        ...
        fn current_dir_policy(network_access: bool) -> SandboxPolicy {
            SandboxPolicy::CurrentDirWrite {
                network_access,
        ...
        fn applies_network_block_when_access_is_disabled() {
            assert!(!current_dir_policy(false).has_full_network_access());
        }

        #[test]
        fn skips_network_block_when_access_is_allowed() {
            assert!(current_dir_policy(true).has_full_network_access());
        }
