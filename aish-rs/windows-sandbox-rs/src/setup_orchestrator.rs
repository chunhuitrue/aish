...
pub const OFFLINE_USERNAME: &str = "AishSandboxOffline";
pub const ONLINE_USERNAME: &str = "AishSandboxOnline";
...
pub(crate) fn gather_write_roots(
    ...
    let mut roots: Vec<PathBuf> = Vec::new();
    // Always include the command CWD for current-dir-write.
    if matches!(policy, SandboxPolicy::CurrentDirWrite { .. }) {
        roots.push(command_cwd.to_path_buf());
    }
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("aish-windows-sandbox-setup.exe");
            if candidate.exists() {
                return candidate;
            }
        }
    }
    PathBuf::from("aish-windows-sandbox-setup.exe")
}
