...
use aish_windows_sandbox::allow_null_device;
use aish_windows_sandbox::convert_string_sid_to_sid;
use aish_windows_sandbox::create_process_as_user;
use aish_windows_sandbox::create_readonly_token_with_cap_from;
use aish_windows_sandbox::create_workspace_write_token_with_cap_from;
use aish_windows_sandbox::get_current_token_for_restriction;
use aish_windows_sandbox::log_note;
use aish_windows_sandbox::parse_policy;
use aish_windows_sandbox::to_wide;
use aish_windows_sandbox::SandboxPolicy;
use anyhow::Context;
use anyhow::Result;
...
struct RunnerRequest {
    ...
    // Writable location for logs (sandbox user's .aish).
    codex_home: PathBuf,
    // Real user's AISH_HOME for shared data (caps, config).
    ...
    let token_res: Result<(HANDLE, *mut c_void)> = unsafe {
        match &policy {
            ...
            SandboxPolicy::CurrentDirWrite { .. } => {
                create_current_dir_write_token_with_cap_from(base, psid_cap)
            }
