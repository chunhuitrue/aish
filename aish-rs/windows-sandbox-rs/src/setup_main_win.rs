...
use aish_windows_sandbox::dpapi_protect;
use aish_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use aish_windows_sandbox::ensure_allow_write_aces;
use aish_windows_sandbox::load_or_create_cap_sids;
use aish_windows_sandbox::log_note;
use aish_windows_sandbox::path_mask_allows;
use aish_windows_sandbox::sandbox_dir;
use aish_windows_sandbox::string_from_sid_bytes;
use aish_windows_sandbox::LOG_FILE_NAME;
use aish_windows_sandbox::SETUP_VERSION;
use anyhow::Context;
use anyhow::Result;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(codex_home) = std::env::var("AISH_HOME") {
