...
# Run a suite of smoke tests against the Windows sandbox via the Aish CLI
...
def _resolve_aish_cmd() -> List[str]:
    """Resolve the Aish CLI to invoke `aish sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Aish.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "aish.exe",
        ws_root / "target" / "release" / "aish.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "aish.exe",
            cargo_base / "release" / "aish.exe",
        ])
    ...
    if shutil.which("aish"):
        return ["aish"]

    raise FileNotFoundError(
        "Aish CLI not found. Build it first, e.g.\n"
        "  cargo build -p aish-cli --release\n"
        "or for debug:\n"
        "  cargo build -p aish-cli\n"
    )

AISH_CMD = _resolve_aish_cmd()
print(AISH_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    argv = [*AISH_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    # 34. WS: policy tamper (.aish artifacts) denied
    codex_home = Path(os.environ["USERPROFILE"]) / ".aish"
    ...
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .aish\\policy.json"], WS_ROOT)
    add("WS: .aish cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .aish policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
