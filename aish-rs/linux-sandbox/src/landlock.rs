...
pub fn is_landlock_supported() -> bool {
    ...
    // Landlock is supported if we can successfully create a ruleset with V1 ABI.
    let abi = ABI::V1;
    Ruleset::default()
...
fn install_filesystem_landlock_rules_on_current_thread(
    ...
    if !is_landlock_supported() {
        return Err(AishErr::Sandbox(SandboxErr::Unavailable));
    }
    let abi = ABI::V5;
    ...
    if status.ruleset == landlock::RulesetStatus::NotEnforced {
        return Err(AishErr::Sandbox(SandboxErr::LandlockRestrict));
    }
