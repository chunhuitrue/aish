#![cfg(target_os = "linux")]
use aish_core::config::types::ShellEnvironmentPolicy;
use aish_core::error::AishErr;
use aish_core::error::SandboxErr;
use aish_core::exec::ExecParams;
use aish_core::exec::process_exec_tool_call;
...
async fn run_cmd(cmd: &[&str], timeout_ms: u64) {
    run_cmd_in_cwd(
        cmd,
...
async fn run_cmd_in_cwd(cmd: &[&str], cwd: PathBuf, timeout_ms: u64) {
    ...
    let sandbox_policy = SandboxPolicy::CurrentDirWrite {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_aish-linux-sandbox");
    let aish_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn test_writable_root() {
    ...
    // Use tmpdir as the cwd so it becomes the writable root
    run_cmd_in_cwd(
        &[
            "bash",
            "-lc",
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_aish-linux-sandbox");
    let aish_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
    ...
    // Verify that we fail with the expected error when landlock is not available.
    if !aish_linux_sandbox::is_landlock_supported() {
        assert!(matches!(
            result,
            Err(AishErr::Sandbox(SandboxErr::Unavailable))
        ));
        return;
    }
    ...
    let output = match result {
        Ok(output) => output,
        Err(AishErr::Sandbox(SandboxErr::Denied { output })) => *output,
        _ => {
