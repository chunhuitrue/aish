...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, AishErr>> {
        ...
        async move {
            match handle.await {
                ...
                Ok(Err(FunctionCallError::Fatal(message))) => Err(AishErr::Fatal(message)),
                Ok(Err(other)) => Err(AishErr::Fatal(other.to_string())),
                Err(err) => Err(AishErr::Fatal(format!(
