...
use aish_protocol::protocol::SkillScope;
...
const ADMIN_SKILLS_ROOT: &str = "/etc/aish/skills";
...
/// Returns the list of skill roots to load from.
/// Only loads from user (~/.aish/skills), system, and admin locations.
pub(crate) fn skill_roots_for_home(codex_home: &Path) -> Vec<SkillRoot> {
    let mut roots = Vec::new();

    // Load order matters: we dedupe by name, keeping the first occurrence.
    // Priority order: user, system, then admin.
...
fn skill_roots(config: &Config) -> Vec<SkillRoot> {
    skill_roots_for_home(&config.codex_home)
}
