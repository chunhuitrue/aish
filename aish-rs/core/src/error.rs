...
use aish_async_utils::CancelErr;
...
pub type Result<T> = std::result::Result<T, AishErr>;
...
pub enum AishErr {
    ...
    #[error(
        "Aish ran out of room in the model's context window. Start a new conversation or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `AishError::Spawn` variant.
    ...
    #[error("Usage not included in your current plan.")]
    UsageNotIncluded,
    ...
    #[error("aish-linux-sandbox was required but not provided")]
    ...
    #[error("Missing base_url for provider '{0}'. Please configure base_url in config.toml.")]
    MissingBaseUrl(String),
}

impl From<CancelErr> for AishErr {
...
impl AishErr {
    /// Minimal shim so that existing `e.downcast_ref::<AishErr>()` checks continue to compile
    ...
    pub fn to_aish_protocol_error(&self) -> AishErrorInfo {
        match self {
            AishErr::ContextWindowExceeded => AishErrorInfo::ContextWindowExceeded,
            AishErr::UsageLimitReached(_) | AishErr::QuotaExceeded | AishErr::UsageNotIncluded => {
                AishErrorInfo::UsageLimitExceeded
            }
            AishErr::RetryLimit(_) => AishErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            AishErr::ConnectionFailed(_) => AishErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            AishErr::ResponseStreamFailed(_) => AishErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            AishErr::RefreshTokenFailed(_) => AishErrorInfo::Unauthorized,
            AishErr::SessionConfiguredNotFirstEvent
            | AishErr::InternalServerError
            | AishErr::InternalAgentDied => AishErrorInfo::InternalServerError,
            AishErr::UnsupportedOperation(_) | AishErr::ConversationNotFound(_) => {
                AishErrorInfo::BadRequest
            }
            AishErr::Sandbox(_) => AishErrorInfo::SandboxError,
            _ => AishErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            codex_error_info: Some(self.to_aish_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            AishErr::RetryLimit(err) => Some(err.status),
            AishErr::UnexpectedStatus(err) => Some(err.status),
            AishErr::ConnectionFailed(err) => err.source.status(),
            AishErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
pub fn get_error_message_ui(e: &AishErr) -> String {
    let message = match e {
        AishErr::Sandbox(SandboxErr::Denied { output }) => {
        ...
        AishErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
...
mod tests {
    ...
    fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
        ...
        let err = AishErr::Sandbox(SandboxErr::Denied {
    ...
    fn to_error_event_handles_response_stream_failed() {
        ...
        let err = AishErr::ResponseStreamFailed(ResponseStreamFailed {
            source,
        ...
        assert_eq!(
            ...
            Some(AishErrorInfo::ResponseStreamConnectionFailed {
