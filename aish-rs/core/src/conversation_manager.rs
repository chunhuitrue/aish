...
use crate::aish::Aish;
use crate::aish::AishSpawnOk;
use crate::aish::INITIAL_SUBMIT_ID;
use crate::aish_conversation::AishConversation;
...
pub struct NewConversation {
    ...
    pub conversation: Arc<AishConversation>,
...
pub struct ConversationManager {
    conversations: Arc<RwLock<HashMap<ConversationId, Arc<AishConversation>>>>,
...
impl ConversationManager {
    ...
    #[cfg(any(test, feature = "test-support"))]
    /// Construct with a dummy AuthManager containing the provided AishAuth.
    /// Used for integration tests: should not be used by ordinary business logic.
    pub fn with_models_provider(auth: AishAuth, provider: ModelProviderInfo) -> Self {
    ...
    #[cfg(any(test, feature = "test-support"))]
    /// Construct with a dummy AuthManager containing the provided AishAuth and codex home.
    /// Used for integration tests: should not be used by ordinary business logic.
    pub fn with_models_provider_and_home(
        ...
        Self {
            ...
            models_manager: Arc::new(ModelsManager::for_testing(auth_manager)),
            skills_manager,
            _test_codex_home_guard: None,
        }
    }
    ...
    pub async fn new_conversation(&self, config: Config) -> AishResult<NewConversation> {
    ...
    pub async fn new_conversation_internal(
        &self,
        config: Config,
        ...
        let AishSpawnOk {
            ...
            session_source.unwrap_or(self.session_source.clone()),
        )
        .await?;
    ...
    async fn finalize_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(AishErr::SessionConfiguredNotFirstEvent);
            }
        };

        let conversation = Arc::new(AishConversation::new(
            codex,
    ...
    pub async fn get_conversation(
        &self,
        conversation_id: ConversationId,
    ) -> AishResult<Arc<AishConversation>> {
        let conversations = self.conversations.read().await;
        conversations
            .get(&conversation_id)
            .cloned()
            .ok_or_else(|| AishErr::ConversationNotFound(conversation_id))
    }
    ...
    /// conversation is stored as `Arc<AishConversation>`, it is possible that
    ...
    pub async fn fork_conversation(
        ...
        let initial_history = InitialHistory::Forked(history.get_rollout_items());
...
mod tests {
    use super::*;
    use crate::aish::make_session_and_context;
