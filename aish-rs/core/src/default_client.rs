...
use aish_client::AishHttpClient;
pub use aish_client::AishRequestBuilder;
...
pub const DEFAULT_ORIGINATOR: &str = "aish_cli_rs";
pub const AISH_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "AISH_INTERNAL_ORIGINATOR_OVERRIDE";
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(AISH_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
    ...
    match HeaderValue::from_str(&value) {
        ...
        Err(err) => {
            tracing::error!("Invalid originator value '{value}': {err}");
            // Fallback to default
            Originator {
...
pub fn create_client() -> AishHttpClient {
    let inner = build_reqwest_client();
    AishHttpClient::new(inner)
}
...
fn is_sandboxed() -> bool {
    std::env::var(AISH_SANDBOX_ENV_VAR).as_deref() == Ok("seatbelt")
}
...
mod tests {
    ...
    fn test_invalid_suffix_is_sanitized() {
        ...
        assert_eq!(
            ...
            format!("{}/0.0.0 (bad_suffix)", originator().value)
        );
    }
