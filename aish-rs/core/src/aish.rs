...
use aish_protocol::approvals::ExecPolicyAmendment;
use aish_protocol::items::TurnItem;
use aish_protocol::protocol::FileChange;
use aish_protocol::protocol::HasLegacyEvent;
use aish_protocol::protocol::ItemCompletedEvent;
use aish_protocol::protocol::ItemStartedEvent;
use aish_protocol::protocol::RawResponseItemEvent;
use aish_protocol::protocol::RolloutItem;
use aish_protocol::protocol::SessionSource;
use aish_protocol::protocol::TaskStartedEvent;
use aish_protocol::protocol::TurnAbortReason;
use aish_protocol::protocol::TurnContextItem;
use aish_rmcp_client::ElicitationResponse;
...
use crate::error::Result as AishResult;
#[cfg(test)]
...
use aish_async_utils::OrCancelExt;
use aish_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use aish_protocol::models::ContentItem;
...
use aish_protocol::protocol::InitialHistory;
use aish_protocol::user_input::UserInput;
use aish_utils_readiness::Readiness;
use aish_utils_readiness::ReadinessFlag;
...
pub struct Aish {
    ...
    pub(crate) rx_events: Receiver<Event>,
}

/// Wrapper returned by [`Aish::spawn`] containing the spawned [`Aish`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct AishSpawnOk {
    pub codex: Aish,
...
impl Aish {
    /// Spawn a new [`Aish`] and initialize the session.
    pub async fn spawn(
        config: Config,
        ...
        let exec_policy = ExecPolicyManager::load(&config.features, &config.config_layer_stack)
            .await
            .map_err(|err| AishErr::Fatal(format!("failed to load execpolicy: {err}")))?;

        let config = Arc::new(config);
        let model = models_manager.get_model(&config.model).ok_or_else(|| {
            AishErr::Fatal(
    ...
    pub async fn submit(&self, op: Op) -> AishResult<String> {
        let id = self
            .next_id
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> AishResult<()> {
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| AishErr::InternalAgentDied)?;
        Ok(())
    }

    pub async fn next_event(&self) -> AishResult<Event> {
        let event = self
            .rx_events
            .recv()
            .await
...
pub(crate) struct TurnContext {
    ...
    pub(crate) aish_linux_sandbox_exe: Option<PathBuf>,
...
impl Session {
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            aish_linux_sandbox_exe: per_turn_config.aish_linux_sandbox_exe.clone(),
    ...
    async fn new(
        ...
        let sandbox_state = SandboxState {
            sandbox_policy: session_configuration.sandbox_policy.get().clone(),
            aish_linux_sandbox_exe: config.aish_linux_sandbox_exe.clone(),
                    ...
                    })
                {
                    ...
                    if prev != curr {
                        ...
                        self.send_event(
                         ...
                         Consider switching back to `{prev}` as it may affect Aish performance."
                                ),
                            }),
                        )
                            .await;
                ...
                Err(err) => {
                    ...
                    self.send_event_raw(Event {
                        ...
                        msg: EventMsg::Error(ErrorEvent {
                            ...
                            codex_error_info: Some(AishErrorInfo::BadRequest),
                        }),
                    })
                    .await;
                    return Err(err);
    ...
    pub(crate) async fn notify_stream_error(
        ...
        let codex_error_info = AishErrorInfo::ResponseStreamDisconnected {
...
async fn submission_loop(sess: Arc<Session>, config: Arc<Config>, rx_sub: Receiver<Submission>) {
    ...
    while let Ok(sub) = rx_sub.recv().await {
        ...
        match sub.op.clone() {
            ...
            Op::ListSkills { force_reload } => {
                handlers::list_skills(&sess, sub.id.clone(), force_reload).await;
            }
            Op::Undo => {
...
mod handlers {
    ...
    use crate::aish::SessionSettingsUpdate;
    ...
    use aish_protocol::custom_prompts::CustomPrompt;
    use aish_protocol::protocol::AishErrorInfo;
    use aish_protocol::protocol::ErrorEvent;
    use aish_protocol::protocol::Event;
    use aish_protocol::protocol::EventMsg;
    use aish_protocol::protocol::ListCustomPromptsResponseEvent;
    use aish_protocol::protocol::ListSkillsResponseEvent;
    use aish_protocol::protocol::Op;
    use aish_protocol::protocol::ReviewDecision;
    use aish_protocol::protocol::TurnAbortReason;
    use aish_protocol::protocol::WarningEvent;

    use aish_protocol::user_input::UserInput;
    use aish_rmcp_client::ElicitationAction;
    use aish_rmcp_client::ElicitationResponse;
    use mcp_types::RequestId;
    use std::sync::Arc;
    use tracing::info;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: aish_protocol::approvals::ElicitationAction,
    ) {
        let action = match decision {
            aish_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            aish_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            aish_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| aish_protocol::message_history::HistoryEntry {
    ...
    pub async fn list_skills(sess: &Session, sub_id: String, force_reload: bool) {
        let (skills, errors) = if sess.enabled(Feature::Skills) {
            let skills_manager = &sess.services.skills_manager;
            let outcome = skills_manager.skills_with_options(force_reload);
            (
                super::skills_to_info(&outcome.skills),
                super::errors_to_info(&outcome.errors),
            )
        } else {
            (Vec::new(), Vec::new())
        };
        let event = Event {
            id: sub_id,
            msg: EventMsg::ListSkillsResponse(ListSkillsResponseEvent { skills, errors }),
        };
        sess.send_event_raw(event).await;
    }
    ...
    pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
        ...
        info!("Shutting down Aish instance");
            ...
            && let Err(e) = rec.shutdown().await
        {
            ...
            let event = Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    codex_error_info: Some(AishErrorInfo::Other),
                }),
            };
            sess.send_event_raw(event).await;
        }
...
pub(crate) async fn run_task(
    ...
    let skills_outcome = sess
        .enabled(Feature::Skills)
        .then(|| sess.services.skills_manager.skills());
    ...
    loop {
        ...
        .await
        {
            ...
            Err(AishErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(AishErr::InvalidImageRequest()) => {
...
async fn run_turn(
    ...
    loop {
        ...
        .await
        {
            ...
            Err(AishErr::TurnAborted) => {
                return Err(AishErr::TurnAborted);
            }
            Err(AishErr::Interrupted) => return Err(AishErr::Interrupted),
            Err(AishErr::EnvVar(var)) => return Err(AishErr::EnvVar(var)),
            Err(e @ AishErr::Fatal(_)) => return Err(e),
            Err(e @ AishErr::ContextWindowExceeded) => {
            ...
            Err(e @ AishErr::QuotaExceeded) => return Err(e),
            Err(e @ AishErr::InvalidImageRequest()) => return Err(e),
            Err(e @ AishErr::InvalidRequest(_)) => return Err(e),
            Err(e @ AishErr::RefreshTokenFailed(_)) => return Err(e),
            Err(e @ AishErr::MissingBaseUrl(_)) => return Err(e),
            Err(e) => {
                ...
                if retries < max_retries {
                    retries += 1;
                    let delay = match e {
                        AishErr::Stream(_, Some(delay)) => delay,
                        _ => backoff(retries),
                    };
                    warn!(
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, AishResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> AishResult<()> {
...
async fn try_run_turn(
    ...
    let mut in_flight: FuturesOrdered<BoxFuture<'static, AishResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: AishResult<TurnRunResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(aish_async_utils::CancelErr::Cancelled) => break Err(AishErr::TurnAborted),
        };

        let event = match event {
            Some(res) => res?,
            None => {
                break Err(AishErr::Stream(
...
mod tests {
    ...
    pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let conversation_id = ConversationId::default();
        let auth_manager =
            AuthManager::from_auth_for_testing(AishAuth::from_api_key("Test API Key"));
