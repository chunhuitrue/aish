...
//!
//! The history is stored at `~/.aish/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.aish`.
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.aish/history.jsonl` and ensure the parent directory exists.
