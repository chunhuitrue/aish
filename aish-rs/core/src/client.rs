...
use aish_api::AggregateStreamExt;
use aish_api::ChatClient as ApiChatClient;
use aish_api::CompactClient as ApiCompactClient;
use aish_api::CompactionInput as ApiCompactionInput;
use aish_api::Prompt as ApiPrompt;
use aish_api::ReqwestTransport;
use aish_api::ResponseStream as ApiResponseStream;
use aish_api::ResponsesClient as ApiResponsesClient;
use aish_api::ResponsesOptions as ApiResponsesOptions;
use aish_api::TransportError;
use aish_api::common::Reasoning;
use aish_api::create_text_param_for_request;
...
use crate::flags::AISH_RS_SSE_FIXTURE;
...
impl ModelClient {
    ...
    async fn stream_chat_completions(&self, prompt: &Prompt) -> Result<ApiResponseStream> {
        if prompt.output_schema.is_some() {
            return Err(AishErr::UnsupportedOperation(
    ...
    async fn stream_responses_api(&self, prompt: &Prompt) -> Result<ResponseStream> {
        if let Some(path) = &*AISH_RS_SSE_FIXTURE {
            warn!(path, "Streaming from fixture");
            let stream = aish_api::stream_from_fixture(path, self.provider.stream_idle_timeout())
                .map_err(map_api_error)?;
            return Ok(map_response_stream(stream));
        }
...
fn map_response_stream<S>(api_stream: S) -> ResponseStream
where
...
/// the mapped `AishErr` is returned to the caller.
async fn handle_unauthorized(
    status: StatusCode,
    _refreshed: &mut bool,
    _auth_manager: &Option<Arc<AuthManager>>,
    _auth: &Option<crate::auth::AishAuth>,
) -> Result<()> {
    // API key authentication does not support token refresh
    Err(map_unauthorized_status(status))
}

fn map_unauthorized_status(status: StatusCode) -> AishErr {
