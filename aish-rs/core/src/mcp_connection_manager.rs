...
//!
//! The [`McpConnectionManager`] owns one [`aish_rmcp_client::RmcpClient`] per
...
use aish_protocol::protocol::McpStartupCompleteEvent;
use aish_protocol::protocol::McpStartupFailure;
use aish_protocol::protocol::McpStartupStatus;
use aish_protocol::protocol::McpStartupUpdateEvent;
use aish_protocol::protocol::SandboxPolicy;
use aish_rmcp_client::ElicitationResponse;
use aish_rmcp_client::OAuthCredentialsStoreMode;
use aish_rmcp_client::RmcpClient;
use aish_rmcp_client::SendElicitation;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
...
pub const MCP_SANDBOX_STATE_CAPABILITY: &str = "aish/sandbox-state";

/// Custom MCP request to push sandbox state updates.
/// When used, the `params` field of the notification is [`SandboxState`].
pub const MCP_SANDBOX_STATE_METHOD: &str = "aish/sandbox-state/update";
...
async fn start_server_task(
    ...
    let params = mcp_types::InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "aish-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Aish".into()),
            // This field is used by Aish when it is an MCP
            // server: it should not be used when Aish is
            // an MCP client.
            user_agent: None,
        },
...
fn mcp_init_error_display(
    ...
    {
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = AISH_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
...
mod tests {
    use super::*;
    use aish_protocol::protocol::McpAuthStatus;
