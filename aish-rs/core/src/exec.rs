...
pub async fn process_exec_tool_call(
    ...
    let exec_env = manager
        .transform(
            ...
            sandbox_type,
            sandbox_cwd,
            aish_linux_sandbox_exe.as_ref(),
        )
        .map_err(AishErr::from)?;
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, AishErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(AishErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(AishErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(AishErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
                }));
            }
...
pub(crate) mod errors {
    ...
    impl From<SandboxTransformError> for AishErr {
        fn from(err: SandboxTransformError) -> Self {
            match err {
                SandboxTransformError::MissingLinuxSandboxExecutable => {
                    AishErr::LandlockSandboxExecutableNotProvided
                }
                #[cfg(not(target_os = "macos"))]
                SandboxTransformError::SeatbeltUnavailable => AishErr::UnsupportedOperation(
...
mod tests {
    ...
    async fn process_exec_tool_call_respects_cancellation_token() -> Result<()> {
        ...
        let output = match result {
            Err(AishErr::Sandbox(SandboxErr::Timeout { output })) => output,
