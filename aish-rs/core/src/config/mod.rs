...
use crate::config::types::SandboxCurrentDirWrite;
...
use crate::project_doc::DEFAULT_INSTRUCTIONS_FILENAME;
...
use aish_rmcp_client::OAuthCredentialsStoreMode;
...
pub use aish_git::GhostSnapshotConfig;
...
pub struct Config {
    ...
    /// User-provided instructions from AISH.md.
    ...
    /// event.
    ///
    /// Example `~/.aish/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// file: AISH_HOME/.credentials.json
    ...
    /// Maximum number of bytes to include from an AISH.md project doc file.
    ...
    /// Directory containing all Codex state (defaults to `~/.aish` but can be
    /// overridden by the `AISH_HOME` environment variable).
    pub codex_home: PathBuf,

    /// Settings that govern if and what will be written to `~/.aish/history.jsonl`.
    pub history: History,
    ...
    /// Path to the `aish-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `aish-linux-sandbox`.
    pub aish_linux_sandbox_exe: Option<PathBuf>,
...
impl Config {
    ...
    /// in [ConfigToml], such as `cwd` and `aish_linux_sandbox_exe`.
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers are only loaded from global config, not from any directory-specific config.
...
/// Base config deserialized from ~/.aish/config.toml.
...
pub struct ConfigToml {
    ...
    /// Sandbox configuration to apply if `sandbox` is `CurrentDirWrite`.
    pub sandbox_current_dir_write: Option<SandboxCurrentDirWrite>,
    ...
    /// Ordered list of fallback filenames to look for when AISH.md is missing.
...
impl ConfigToml {
    ...
    fn derive_sandbox_policy(
        ...
        let mut sandbox_policy = match resolved_sandbox_mode {
            ...
            SandboxMode::CurrentDirWrite => match self.sandbox_current_dir_write.as_ref() {
                Some(SandboxCurrentDirWrite {
                    network_access,
                ...
                None => SandboxPolicy::new_current_dir_write_policy(),
            },
            SandboxMode::DangerFullAccess => SandboxPolicy::DangerFullAccess,
        };
        let mut forced_auto_mode_downgraded_on_windows = false;
        if cfg!(target_os = "windows")
            && matches!(resolved_sandbox_mode, SandboxMode::CurrentDirWrite)
...
impl Config {
    ...
    fn load_config_with_layer_stack(
        ...
        let approval_policy = approval_policy_override
            ...
            .unwrap_or({
                // Global default: require user approval for all operations
    ...
    fn load_instructions(codex_dir: Option<&Path>) -> Option<String> {
        ...
        path.push(DEFAULT_INSTRUCTIONS_FILENAME);
...
/// specified by the `AISH_HOME` environment variable. If not set, defaults to
/// `~/.aish`.
///
/// - If `AISH_HOME` is set, the value will be canonicalized and this
///   function will Err if the path does not exist.
/// - If `AISH_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_codex_home() -> std::io::Result<PathBuf> {
    // Honor the `AISH_HOME` environment variable when it is set to allow users
    // (and tests) to override the default location.
    if let Ok(val) = std::env::var("AISH_HOME")
        && !val.is_empty()
    {
...
/// Returns the path to the folder where Aish logs are stored. Does not verify
...
"#;
        ...
        let sandbox_current_dir_write = r#"
sandbox_mode = "current-dir-write"
...
"#;
        ...
        let sandbox_current_dir_write_cfg = toml::from_str::<ConfigToml>(sandbox_current_dir_write)
            .expect("TOML deserialization should succeed");
        let sandbox_mode_override = None;
        let resolution = sandbox_current_dir_write_cfg.derive_sandbox_policy(
            sandbox_mode_override,
            None,
            &PathBuf::from("/tmp/test"),
        );
        if cfg!(target_os = "windows") {
            ...
            assert_eq!(
                resolution,
                SandboxPolicyResolution {
                    policy: SandboxPolicy::CurrentDirWrite {
                        network_access: true,
                        exclude_tmpdir_env_var: true,
                        exclude_slash_tmp: true,
                    },
    ...
    #[test]
    fn current_dir_write_sandbox_includes_cwd() -> std::io::Result<()> {
        ...
        let overrides = ConfigOverrides {
            ...
            sandbox_mode: Some(SandboxMode::CurrentDirWrite),
            ..Default::default()
        };
        ...
        let expected_frontend = AbsolutePathBuf::try_from(frontend.clone()).unwrap();
        if cfg!(target_os = "windows") {
            assert!(
                config.forced_auto_mode_downgraded_on_windows,
                "expected current-dir-write request to be downgraded on Windows"
            );
            ...
            match config.sandbox_policy.get() {
                SandboxPolicy::CurrentDirWrite { .. } => {
                    let writable_roots = config
                        .sandbox_policy
                        .get()
                        .get_writable_roots_with_cwd(&frontend);
                    assert!(
                        writable_roots
                            .iter()
                            .any(|root| root.root == expected_frontend),
                        "expected cwd to be in writable roots"
                    );
                }
                other => panic!("expected current-dir-write policy, got {other:?}"),
            }
        }
...
"#
    ...
    async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
        ...
        let cwd_path = PathBuf::from("/tmp/aish-mcp");
        let servers = BTreeMap::from([(
            "docs".to_string(),
            McpServerConfig {
                transport: McpServerTransportConfig::Stdio {
        ...
        let serialized = std::fs::read_to_string(&config_path)?;
        assert!(
            serialized.contains(r#"cwd = "/tmp/aish-mcp""#),
        ...
        match &docs.transport {
            McpServerTransportConfig::Stdio { cwd, .. } => {
                assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/aish-mcp")));
            }
            other => panic!("unexpected transport {other:?}"),
        }
...
"#;
        ...
        // Make it look like a Git repo so it does not search for AISH.md in
