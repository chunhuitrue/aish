...
#[cfg(unix)]
const AISH_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/aish/managed_config.toml";

/// If present, data read from a file such as `/etc/aish/managed_config.toml`.
#[derive(Debug, Clone, PartialEq)]
pub struct MangedConfigFromFile {
...
/// Return the default managed config path (honoring `AISH_MANAGED_CONFIG_PATH`).
pub(super) fn managed_config_default_path(codex_home: &Path) -> PathBuf {
    if let Ok(path) = std::env::var("AISH_MANAGED_CONFIG_PATH") {
    ...
    {
        let _ = codex_home;
        PathBuf::from(AISH_MANAGED_CONFIG_SYSTEM_PATH)
    }
