...
/// Base config deserialized from /etc/aish/requirements.toml or MDM.
...
impl From<SandboxMode> for SandboxModeRequirement {
    fn from(mode: SandboxMode) -> Self {
        match mode {
            ...
            SandboxMode::CurrentDirWrite => SandboxModeRequirement::CurrentDirWrite,
...
impl TryFrom<ConfigRequirementsToml> for ConfigRequirements {
    ...
    fn try_from(toml: ConfigRequirementsToml) -> Result<Self, Self::Error> {
        ...
        // the other variants (CurrentDirWrite, ExternalSandbox) require
                ...
                Constrained::new(default_sandbox_policy, move |candidate| {
                    let mode = match candidate {
                        ...
                        SandboxPolicy::CurrentDirWrite { .. } => {
                            SandboxModeRequirement::CurrentDirWrite
                        }
...
mod tests {
    use super::*;
    use aish_protocol::protocol::NetworkAccess;
    use anyhow::Result;
    ...
    fn deserialize_allowed_sandbox_modes() -> Result<()> {
        let toml_str = r#"
            allowed_sandbox_modes = ["read-only", "current-dir-write"]
        "#;
        let config: ConfigRequirementsToml = from_str(toml_str)?;
        let requirements = ConfigRequirements::try_from(config)?;
        ...
        assert!(
            requirements
                .sandbox_policy
                .can_set(&SandboxPolicy::CurrentDirWrite {
        ...
        assert_eq!(
            ...
            Err(ConstraintError::InvalidValue {
                ...
                allowed: "[ReadOnly, CurrentDirWrite]".into(),
            })
        );
        assert_eq!(
            requirements
