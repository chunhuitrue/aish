...
use aish_core::LocalShellAction;
use aish_core::LocalShellExecAction;
use aish_core::LocalShellStatus;
...
use aish_protocol::models::ReasoningItemContent;
use aish_protocol::models::ReasoningItemReasoningSummary;
use aish_protocol::models::WebSearchAction;
...
use core_test_support::test_aish::TestAish;
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = aish_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![aish_protocol::models::ContentItem::InputText {
    ...
    let prior_system = aish_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![aish_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = aish_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let auth_manager =
        aish_core::AuthManager::from_auth_for_testing(AishAuth::from_api_key("Test API Key"));
    let NewConversation {
        conversation: codex,
        session_configured,
        ..
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "aish_cli_rs");
    assert_eq!(
...
async fn includes_user_instructions_message_in_request() {
    ...
    assert_message_starts_with(&request_body["input"][0], "# AISH.md instructions for ");
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    // Use a model that doesn't have default_reasoning_effort (default fallback)
    let TestAish { codex, .. } = test_aish().with_model("any-model").build(&server).await?;
...
-> anyhow::Result<()> {
    ...
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    let TestAish { codex, .. } = test_aish().with_model("test-model").build(&server).await?;
...
async fn configured_verbosity_not_sent_for_models_without_support() -> anyhow::Result<()> {
    ...
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    // Use a model that doesn't support verbosity (default fallback)
    let TestAish { codex, .. } = test_aish()
        .with_model("any-model")
        .with_config(|config| {
...
async fn includes_developer_instructions_message_in_request() {
    ...
    assert_message_role(&request_body["input"][1], "user");
    assert_message_starts_with(&request_body["input"][1], "# AISH.md instructions for ");
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let _auth_manager = AuthManager::from_auth_for_testing(AishAuth::from_api_key("Test API Key"));
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is "test-model" which has no context window defined
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let TestAish { codex, .. } = test_aish()
        .with_config(|config| {
            config.model = Some("exp-5.1".to_string());
    ...
    let expected_context_window_message = AishErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_codex_auth() -> AishAuth {
