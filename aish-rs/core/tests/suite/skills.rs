...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    test.codex
        .submit(Op::UserTurn {
            ...
            model: session_model,
            effort: None,
            summary: aish_protocol::config_types::ReasoningSummary::Auto,
        })
        .await?;

    core_test_support::wait_for_event(test.codex.as_ref(), |event| {
        matches!(event, aish_core::protocol::EventMsg::TaskComplete(_))
    })
    .await;
...
async fn skill_load_errors_surface_in_session_configured() -> Result<()> {
    ...
    let response =
        core_test_support::wait_for_event_match(test.codex.as_ref(), |event| match event {
            aish_core::protocol::EventMsg::ListSkillsResponse(response) => Some(response.clone()),
            _ => None,
        })
        .await;
...
async fn list_skills_includes_system_cache_entries() -> Result<()> {
    ...
    test.codex
        .submit(Op::ListSkills { force_reload: true })
        .await?;
    let response =
        core_test_support::wait_for_event_match(test.codex.as_ref(), |event| match event {
            aish_core::protocol::EventMsg::ListSkillsResponse(response) => Some(response.clone()),
    ...
    assert_eq!(skill.scope, aish_protocol::protocol::SkillScope::System);
