...
async fn stdio_image_completions_round_trip() -> anyhow::Result<()> {
    ...
    let fixture = test_aish()
        .with_config(move |config| {
            config.model_provider.wire_api = aish_core::WireApi::Chat;
            config.mcp_servers.insert(
                server_name.to_string(),
                McpServerConfig {
                    transport: McpServerTransportConfig::Stdio {
...
/// This test writes to a fallback credentials file in AISH_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire AISH_HOME through the code.
#[serial(codex_home)]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("AISH_HOME", temp_home.path().as_os_str());
