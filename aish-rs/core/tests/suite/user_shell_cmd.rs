...
use aish_core::protocol::ExecOutputStream;
...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    let conversation_manager = ConversationManager::with_models_provider(
        aish_core::AishAuth::from_api_key("dummy"),
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_aish::test_aish();
    let test = builder.build(&server).await?;

    #[cfg(windows)]
    let command = r#"$val = $env:AISH_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${AISH_SANDBOX:-not-set}\"""#.to_string();
