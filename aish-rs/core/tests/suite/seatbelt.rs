...
use aish_core::spawn::AISH_SANDBOX_ENV_VAR;
...
impl TestScenario {
    async fn run_test(
        ...
        if std::env::var(AISH_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
            eprintln!("{AISH_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
            return;
        }

        assert_eq!(
            touch(&self.file_outside_repo, sandbox_cwd, policy).await,
        ...
        assert_eq!(
            touch(&self.file_in_repo_root, sandbox_cwd, policy).await,
        ...
        assert_eq!(
            touch(&self.file_in_dot_git_dir, sandbox_cwd, policy).await,
...
/// Git cannot be used to backup their work before the agent begins.
///
...
#[tokio::test]
async fn if_cwd_is_repo_root_then_dot_git_folder_is_read_only() {
...
async fn java_home_finds_runtime_under_seatbelt() {
    ...
    env.remove(AISH_SANDBOX_ENV_VAR);
...
fn create_test_scenario(tmp: &TempDir) -> TestScenario {
    // Use tmp.path() as the repo_root since CurrentDirWrite uses cwd as writable root
    let repo_root = tmp.path();
    let dot_git_dir = repo_root.join(".git");

    std::fs::create_dir(&dot_git_dir).expect("should be able to create .git dir");

    // Create a separate temp directory for files outside the sandbox
    let outside_dir = TempDir::new().expect("should be able to create outside temp dir");
    let file_outside_repo = outside_dir.path().join("outside.txt");
...
async fn touch(path: &Path, sandbox_cwd: &Path, policy: &SandboxPolicy) -> bool {
