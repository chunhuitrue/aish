...
use core_test_support::test_aish::ApplyPatchModelOutput;
...
use core_test_support::test_aish::TestAishBuilder;
use core_test_support::test_aish::TestAishHarness;
...
pub async fn apply_patch_harness() -> Result<TestAishHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestAishBuilder) -> TestAishBuilder,
) -> Result<TestAishHarness> {
    let builder = configure(test_aish()).with_config(|config| {
        config.include_apply_patch_tool = true;
    });
    TestAishHarness::with_builder(builder).await
}

pub async fn mount_apply_patch(
    harness: &TestAishHarness,
    call_id: &str,
    patch: &str,
    assistant_msg: &str,
