...
use aish_core::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
use aish_core::protocol::EventMsg;
use aish_core::protocol::Op;
use aish_core::protocol::SandboxPolicy;
use aish_core::protocol_config_types::ReasoningSummary;
use aish_core::shell::Shell;
use aish_core::shell::default_user_shell;
...
async fn codex_mini_latest_tools() -> anyhow::Result<()> {
    ...
    // test-model does not need apply_patch_tool_instructions
...
-> anyhow::Result<()> {
    ...
    let TestAish { codex, config, .. } = test_aish()
        .with_config(|config| {
            config.user_instructions = Some("be consistent and helpful".to_string());
        })
        .build(&server)
...
async fn overrides_turn_context_but_keeps_cached_prefix_and_key_constant() -> anyhow::Result<()> {
    ...
    codex
        .submit(Op::OverrideTurnContext {
            ...
            sandbox_policy: Some(SandboxPolicy::CurrentDirWrite {
                network_access: true,
...
async fn override_before_first_turn_emits_environment_context() -> anyhow::Result<()> {
    ...
    let TestAish { codex, .. } = test_aish().build(&server).await?;
...
async fn per_turn_overrides_keep_cached_prefix_and_key_constant() -> anyhow::Result<()> {
    ...
    codex
        .submit(Op::UserTurn {
            ...
            sandbox_policy: SandboxPolicy::CurrentDirWrite {
                network_access: true,
                exclude_tmpdir_env_var: true,
                exclude_slash_tmp: true,
            },
