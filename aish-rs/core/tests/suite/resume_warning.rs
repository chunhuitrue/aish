...
use aish_core::protocol::InitialHistory;
use aish_core::protocol::ResumedHistory;
use aish_core::protocol::RolloutItem;
use aish_core::protocol::TurnContextItem;
...
fn resume_history(
    config: &aish_core::config::Config,
    previous_model: &str,
...
async fn emits_warning_when_resumed_model_differs() {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(AishAuth::from_api_key("test"));
