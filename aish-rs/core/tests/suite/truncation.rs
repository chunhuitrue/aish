...
async fn truncate_function_error_trims_respond_to_model() -> Result<()> {
    ...
    let mut builder = test_aish().with_model("test-model");
...
async fn tool_call_output_configured_limit_chars_type() -> Result<()> {
    ...
    let mut builder = test_aish().with_model("test-model").with_config(|config| {
    ...
    // With 100k token limit and test-model using token-based truncation,
    // the output may still be truncated if it exceeds the limit
    let len = output.len();
    assert!(
        len > 100_000,
        "shell output should have significant content, got {len} chars"
    );
...
async fn tool_call_output_exceeds_limit_truncated_chars_limit() -> Result<()> {
    ...
    let len = output.len();
    // test-model uses TruncationPolicy::Tokens(10_000), which is ~40k chars
    assert!(
        (39_000..=42_000).contains(&len),
        "expected ~40k chars after token truncation, got {len}"
    );
...
async fn mcp_tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    let mut builder = test_aish().with_config(move |config| {
        config.mcp_servers.insert(
            ...
            aish_core::config::types::McpServerConfig {
                transport: aish_core::config::types::McpServerTransportConfig::Stdio {
                ...
                enabled: true,
                startup_timeout_sec: Some(std::time::Duration::from_secs(60)),
