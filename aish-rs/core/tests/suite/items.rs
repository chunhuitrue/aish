...
use aish_core::protocol::ItemCompletedEvent;
use aish_core::protocol::ItemStartedEvent;
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let Some(aish_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {
