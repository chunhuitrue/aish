...
fn network_disabled() -> bool {
    std::env::var(AISH_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (TempDir, Config, ConversationManager, Arc<AishConversation>) {
...
async fn user_turn(conversation: &Arc<AishConversation>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<AishConversation>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn fetch_conversation_path(conversation: &Arc<AishConversation>) -> std::path::PathBuf {
...
) -> Arc<AishConversation> {
    let auth_manager =
        aish_core::AuthManager::from_auth_for_testing(AishAuth::from_api_key("dummy"));
    let NewConversation { conversation, .. } = manager
...
async fn fork_conversation(
    ...
    let NewConversation { conversation, .. } = manager
        .fork_conversation(config.clone(), path, nth_user_message)
        .await
