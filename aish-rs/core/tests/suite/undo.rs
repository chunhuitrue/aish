...
async fn undo_harness() -> Result<TestAishHarness> {
    let builder = test_aish().with_model("gpt-5.1").with_config(|config| {
...
async fn invoke_undo(codex: &Arc<AishConversation>) -> Result<UndoCompletedEvent> {
...
async fn expect_successful_undo(codex: &Arc<AishConversation>) -> Result<UndoCompletedEvent> {
...
async fn expect_failed_undo(codex: &Arc<AishConversation>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(codex).await?;
    assert!(
        !event.success,
