...
async fn grep_files_tool_collects_matches() -> Result<()> {
    ...
    let test = build_test_aish(&server).await?;
...
async fn build_test_aish(server: &wiremock::MockServer) -> Result<TestAish> {
    let mut builder = test_aish().with_model(MODEL_WITH_TOOL);
