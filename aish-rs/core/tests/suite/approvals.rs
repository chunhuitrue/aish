...
use aish_core::config::Constrained;
...
use aish_core::protocol::ExecPolicyAmendment;
use aish_core::protocol::Op;
use aish_core::protocol::SandboxPolicy;
use aish_core::sandboxing::SandboxPermissions;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestAish) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestAish, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(test: &TestAish, expected_command: &str) -> ExecApprovalRequestEvent {
...
async fn wait_for_completion_without_approval(test: &TestAish) {
    let event = wait_for_event(&test.codex, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TaskComplete(_)
...
async fn wait_for_completion(test: &TestAish) {
...
fn scenarios() -> Vec<ScenarioSpec> {
    ...
    let current_dir_write = |network_access| SandboxPolicy::CurrentDirWrite {
        network_access,
    ...
    vec![
        ...
        ScenarioSpec {
            ...
            approval_policy: UnlessTrusted,
            sandbox_policy: current_dir_write(false),
        ...
        ScenarioSpec {
            ...
            expectation: Expectation::FileNotCreated {
                target: TargetPath::OutsideWorkspace("apply_patch_function_never.txt"),
                message_contains: &[
                    "patch rejected: writing to a path outside of the current working directory; rejected by user approval settings",
                ],
            },
        },
        ScenarioSpec {
        ...
        ScenarioSpec {
            name: "current_dir_write_on_request_allows_current_dir_write",
            approval_policy: OnRequest,
            sandbox_policy: current_dir_write(false),
            action: ActionKind::WriteFile {
        ...
        ScenarioSpec {
            name: "current_dir_write_network_disabled_blocks_network",
            approval_policy: OnRequest,
            sandbox_policy: current_dir_write(false),
            action: ActionKind::FetchUrl {
        ...
        ScenarioSpec {
            name: "current_dir_write_on_request_requires_approval_outside_workspace",
            approval_policy: OnRequest,
            sandbox_policy: current_dir_write(false),
            action: ActionKind::WriteFile {
        ...
        ScenarioSpec {
            name: "current_dir_write_network_enabled_allows_network",
            approval_policy: OnRequest,
            sandbox_policy: current_dir_write(true),
            action: ActionKind::FetchUrl {
        ...
        #[cfg(not(target_os = "linux"))] // TODO (pakrym): figure out why linux behaves differently
        ScenarioSpec {
            name: "current_dir_write_on_failure_escalates_outside_workspace",
            approval_policy: OnFailure,
            sandbox_policy: current_dir_write(false),
            action: ActionKind::WriteFile {
        ...
        ScenarioSpec {
            name: "current_dir_write_unless_trusted_requires_approval_outside_workspace",
            approval_policy: UnlessTrusted,
            sandbox_policy: current_dir_write(false),
            action: ActionKind::WriteFile {
        ...
        ScenarioSpec {
            name: "current_dir_write_never_blocks_outside_workspace",
            approval_policy: Never,
            sandbox_policy: current_dir_write(false),
            action: ActionKind::WriteFile {
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let model = model_override.unwrap_or("exp-5.1");

    let mut builder = test_aish().with_model(model).with_config(move |config| {
...
async fn approving_execpolicy_amendment_persists_policy_and_skips_future_prompts() -> Result<()> {
    ...
    let mut builder = test_aish().with_config(move |config| {
        config.approval_policy = Constrained::allow_any(approval_policy);
