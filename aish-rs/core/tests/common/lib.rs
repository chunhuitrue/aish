...
use aish_core::AishConversation;
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\aish\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer's real `~/.aish`.
pub async fn load_default_config_for_test(codex_home: &TempDir) -> Config {
    ...
    // Set a default test model since built-in presets have been removed.
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        aish_linux_sandbox_exe: Some(
            aish_utils_cargo_bin::cargo_bin("aish-linux-sandbox")
                .expect("should find binary for aish-linux-sandbox"),
        ),
...
pub async fn wait_for_event<F>(
    codex: &AishConversation,
    predicate: F,
) -> aish_core::protocol::EventMsg
where
    F: FnMut(&aish_core::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(codex, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(codex: &AishConversation, matcher: F) -> T
where
    F: Fn(&aish_core::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    aish_core::spawn::AISH_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    aish_core::spawn::AISH_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    aish_core::shell::default_user_shell().derive_exec_args(command, true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    aish_core::shell::default_user_shell().derive_exec_args(command, false)
}
...
pub mod fs_wait {
    ...
    fn wait_for_path_exists_blocking(path: PathBuf, timeout: Duration) -> Result<PathBuf> {
        ...
        // If recursive watching is expensive or failing, we can try NonRecursive.
        // But for sessions/YYYY/MM/DD structure, recursive is safer.
    ...
    fn blocking_find_matching_file(
        ...
        let (tx, rx) = mpsc::channel();
        let mut watcher = notify::recommended_watcher(move |res| {
            // It's common for watchers to error or events to be missed on CI or busy systems.
            // We just ignore send errors.
            let _ = tx.send(res);
        })?;
        // If recursive watching is expensive or failing, we can try NonRecursive.
        // But for sessions/YYYY/MM/DD structure, recursive is safer.
