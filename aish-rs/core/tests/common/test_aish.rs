...
pub struct TestAishBuilder {
    config_mutators: Vec<Box<ConfigMutator>>,
    auth: AishAuth,
    pre_build_hooks: Vec<Box<PreBuildHook>>,
}

impl TestAishBuilder {
    ...
    pub fn with_auth(mut self, auth: AishAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestAish> {
    ...
    async fn build_from_config(
        ...
        let new_conversation = match resume_from {
            Some(path) => {
                let auth_manager = aish_core::AuthManager::from_auth_for_testing(auth);
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = aish_utils_cargo_bin::cargo_bin("aish") {
            config.aish_linux_sandbox_exe = Some(path);
        }
...
pub struct TestAish {
    ...
    pub cwd: Arc<TempDir>,
    pub codex: Arc<AishConversation>,
...
pub struct TestAishHarness {
    server: MockServer,
    test: TestAish,
}

impl TestAishHarness {
    pub async fn new() -> Result<Self> {
        Self::with_builder(test_aish()).await
    }

    pub async fn with_config(mutator: impl FnOnce(&mut Config) + Send + 'static) -> Result<Self> {
        Self::with_builder(test_aish().with_config(mutator)).await
    }

    pub async fn with_builder(mut builder: TestAishBuilder) -> Result<Self> {
...
pub fn test_aish() -> TestAishBuilder {
    TestAishBuilder {
        config_mutators: vec![],
        auth: AishAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
    }
}
