...
const DEFAULT_COMMIT_MESSAGE: &str = "aish snapshot";
...
pub fn create_ghost_commit_with_report(
    ...
    let large_untracked_dirs = existing_untracked
        ...
        .collect::<Vec<_>>();
    let index_tempdir = Builder::new().prefix("aish-git-index-").tempdir()?;
...
fn default_commit_identity() -> Vec<(OsString, OsString)> {
    vec![
        (
            ...
            OsString::from("Aish Snapshot"),
        ),
        (
            OsString::from("GIT_AUTHOR_EMAIL"),
            OsString::from("snapshot@aish.local"),
        ),
        (
