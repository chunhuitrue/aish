...
use aish_core::config::load_global_mcp_servers;
use aish_core::config::types::McpServerConfig;
use aish_core::config::types::McpServerTransportConfig;
use aish_core::mcp::auth::compute_auth_statuses;
use aish_core::protocol::McpAuthStatus;
use aish_rmcp_client::delete_oauth_tokens;
use aish_rmcp_client::perform_oauth_login;
use aish_rmcp_client::supports_oauth_login;
use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use anyhow::bail;
...
/// - `add`    — add a server launcher entry to `~/.aish/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "aish mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let codex_home = find_codex_home().context("failed to resolve AISH_HOME")?;
    ...
    } = transport
    {
        match supports_oauth_login(&url).await {
            ...
            Ok(false) => {}
            Err(_) => println!(
                "MCP server may or may not require login. Run `aish mcp login {name}` to login."
            ),
        }
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `aish mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: aish mcp remove {}", get_args.name);
