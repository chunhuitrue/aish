...
///
/// Note: The `--add-dir` option is deprecated and no longer has any effect,
/// as writable roots are automatically determined based on the current working
/// directory.
...
fn format_warning(additional_dirs: &[PathBuf]) -> String {
    let joined_paths = additional_dirs
        ...
        .join(", ");
    format!(
        "Ignoring --add-dir ({joined_paths}) because the effective sandbox mode is read-only. Switch to current-dir-write or danger-full-access to allow write access to the current directory."
    )
}
...
mod tests {
    ...
    #[test]
    fn returns_none_for_current_dir_write() {
        let sandbox = SandboxPolicy::new_current_dir_write_policy();
    ...
    fn warns_for_read_only() {
        ...
        assert_eq!(
            message,
            "Ignoring --add-dir (relative, /abs) because the effective sandbox mode is read-only. Switch to current-dir-write or danger-full-access to allow write access to the current directory."
        );
    }
