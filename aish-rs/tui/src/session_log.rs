...
pub(crate) fn maybe_init(config: &Config) {
    let enabled = std::env::var("AISH_TUI_RECORD_SESSION")
    ...
    let path = if let Ok(path) = std::env::var("AISH_TUI_SESSION_LOG_PATH") {
        PathBuf::from(path)
    } else {
        let mut p = match aish_core::config::log_dir(config) {
            Ok(dir) => dir,
