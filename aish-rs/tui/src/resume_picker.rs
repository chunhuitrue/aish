...
use aish_core::ConversationItem;
use aish_core::ConversationsPage;
use aish_core::Cursor;
use aish_core::INTERACTIVE_SESSION_SOURCES;
use aish_core::RolloutRecorder;
use aish_core::path_utils;
...
use aish_protocol::protocol::SessionMetaLine;
...
pub async fn run_resume_picker(
    ...
    // Always show all sessions without CWD filtering
    let filter_cwd: Option<PathBuf> = None;
...
fn head_to_row(item: &ConversationItem) -> Row {
    ...
    let cwd = extract_session_meta_from_head(&item.head);
...
fn extract_session_meta_from_head(head: &[serde_json::Value]) -> Option<PathBuf> {
    for value in head {
...
fn preview_from_head(head: &[serde_json::Value]) -> Option<String> {
    head.iter()
        ...
        .find_map(|item| match aish_core::parse_turn_item(&item) {
...
fn render_list(
    ...
    for (idx, (row, (updated_label, cwd_label))) in rows[start..end]
        .iter()
        ...
        .enumerate()
    {
        ...
        let add_leading_gap = max_updated_width == 0 && max_cwd_width == 0;
        if add_leading_gap {
...
fn calculate_column_metrics(rows: &[Row]) -> ColumnMetrics {
    ...
    // Always show CWD column since we always show all sessions
    ...
    fn preview_uses_first_message_input_text() {
        let head = vec![
            ...
            json!({
                ...
                "role": "user",
                "content": [
                    { "type": "input_text", "text": "# AISH.md instructions for project\n\n<INSTRUCTIONS>\nhi\n</INSTRUCTIONS>" },
                ]
            }),
            json!({
                "type": "message",
