...
use aish_core::config::ConstraintError;
...
use aish_core::protocol::TokenCountEvent;
...
fn set_windows_sandbox_enabled(enabled: bool) {
    aish_core::set_windows_sandbox_enabled(enabled);
}
...
async fn resumed_initial_messages_render_history() {
    ...
    let configured = aish_core::protocol::SessionConfiguredEvent {
...
async fn make_chatwidget_manual(
    ...
    let resolved_model = model_override
        .map(str::to_owned)
        .or_else(|| ModelsManager::get_model_offline(cfg.model.as_deref()))
        .unwrap_or_else(|| "test-model".to_string());
...
fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> = aish_core::parse_command::parse_command(&command);
...
async fn exec_end_without_begin_uses_event_command() {
    ...
    let parsed_cmd = aish_core::parse_command::parse_command(&command);
    let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    chat.handle_codex_event(Event {
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    let rollout_path = PathBuf::from("/tmp/aish-test-rollout.jsonl");
...
async fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_codex_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(aish_core::protocol::TurnAbortedEvent {
...
#[tokio::test]
async fn preset_matching_with_current_dir_write() {
    let preset = builtin_approval_presets()
        .into_iter()
        .find(|p| p.id == "auto")
        .expect("auto preset exists");
    let current_sandbox = SandboxPolicy::CurrentDirWrite {
    ...
    assert!(
        ...
        "CurrentDirWrite should match the Agent preset"
    );
    assert!(
...
async fn disabled_slash_command_while_task_running_snapshot() {
    ...
    // Dispatch a command that is unavailable while a task runs (e.g., /approvals)
...
async fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CodexOp(Op::PatchApproval { id, decision }) = app_ev {
            ...
            assert_matches!(decision, aish_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
