...
use aish_common::format_env_display::format_env_display;
use aish_core::config::Config;
use aish_core::config::types::McpServerTransportConfig;
use aish_core::protocol::FileChange;
use aish_core::protocol::McpAuthStatus;
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: aish_core::protocol::ReviewDecision,
) -> Box<dyn HistoryCell> {
    use aish_core::protocol::ReviewDecision::*;
...
pub(crate) fn new_session_info(
    ...
    let header = SessionHeaderHistoryCell::new(model.clone(), reasoning_effort, AISH_CLI_VERSION);
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/approvals".into(),
                " - choose what Aish can do without approval".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/review".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, _width: u16) -> Vec<Line<'static>> {
        ...
        // 添加ASCII艺术字
        for art_line in AISH_ASCII_ART {
            lines.push(Line::from(Span::from(*art_line).green().bold()));
        }

        // 构建版本和模型信息
        let mut info = format!("v{}  {}", self.version, self.model);
        if let Some(reasoning) = self.reasoning_label() {
            info.push_str(&format!("  {reasoning}"));
        }

        // 计算居中所需的空格数
        let art_width = AISH_ASCII_ART
            .iter()
            .map(|s| UnicodeWidthStr::width(*s))
            .max()
            .unwrap_or(0);
        let info_width = UnicodeWidthStr::width(info.as_str());
        let padding = if art_width > info_width {
            (art_width - info_width) / 2
        } else {
            0
        };

        // 添加居中的信息行
        let padded_info = format!("{}{}", " ".repeat(padding), info);
        lines.push(Line::from(Span::from(padded_info).dim()));
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        "    MCP servers can be configured in your config file."
            .italic()
            .into(),
    ];
...
impl HistoryCell for FinalMessageSeparator {
    fn display_lines(&self, _width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(elapsed_seconds) = elapsed_seconds {
            let worked_for = format!("─ Took {elapsed_seconds}");
            vec![Line::from_iter([worked_for]).dim()]
        } else {
            Vec::new()
        }
    }
...
mod tests {
    ...
    use aish_core::config::types::McpServerConfig;
    use aish_core::config::types::McpServerTransportConfig;
    use aish_core::protocol::McpAuthStatus;
    ...
    fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
        ...
        assert_eq!(
            rendered,
            vec![
                "✔ You approved aish to".to_string(),
    ...
    fn session_header_includes_reasoning_level_when_present() {
        ...
        // 新格式: "v{version}  {model}  {reasoning}"
        let info_line = lines
            .into_iter()
            .find(|line| line.contains("gpt-4o"))
            .expect("info line");

        assert!(info_line.contains("vtest"));
        assert!(info_line.contains("high"));
    }
