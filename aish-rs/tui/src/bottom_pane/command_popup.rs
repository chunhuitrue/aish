...
mod tests {
    ...
    #[test]
    fn filter_includes_new_when_typing_prefix() {
        let mut popup = CommandPopup::new(Vec::new(), false);
        // Simulate the composer line starting with '/ne' so the popup filters
        // matching commands by prefix.
        popup.on_composer_text_change("/ne".to_string());

        // Access the filtered list via the selected command and ensure that
        // one of the matches is the new "new" command.
        let matches = popup.filtered_items();
        let has_new = matches.iter().any(|item| match item {
            CommandItem::Builtin(cmd) => cmd.command() == "new",
            CommandItem::UserPrompt(_) => false,
        });
        assert!(has_new, "expected '/new' to appear among filtered commands");
    }

    #[test]
    fn selecting_new_by_exact_match() {
        let mut popup = CommandPopup::new(Vec::new(), false);
        ...
        match selected {
            Some(CommandItem::Builtin(cmd)) => assert_eq!(cmd.command(), "new"),
            Some(CommandItem::UserPrompt(_)) => panic!("unexpected prompt selected for '/new'"),
    ...
    fn prompt_name_collision_with_builtin_is_ignored() {
        // Create a prompt named like a builtin (e.g. "new").
        let popup = CommandPopup::new(
            vec![CustomPrompt {
                name: "new".to_string(),
                path: "/tmp/new.md".to_string().into(),
        ...
        let has_collision_prompt = items.into_iter().any(|it| match it {
            CommandItem::UserPrompt(i) => popup.prompt(i).is_some_and(|p| p.name == "new"),
            _ => false,
        });
        assert!(
    ...
    fn fuzzy_filter_matches_subsequence_for_ac() {
        ...
        assert_eq!(
            cmds,
            vec!["compact"],
            "expected fuzzy search for '/ac' to match only compact, got {cmds:?}"
        );
    }
}
