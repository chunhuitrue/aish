...
use aish_common::fuzzy_match::fuzzy_match;
use aish_protocol::custom_prompts::CustomPrompt;
use aish_protocol::custom_prompts::PROMPTS_CMD_PREFIX;
    ...
    fn handle_key_event_without_popup(&mut self, key_event: KeyEvent) -> (InputResult, bool) {
        ...
        match key_event {
            ...
            KeyEvent {
                ...
                // the workflow: type a prefix ("/ex"), press Tab to complete to
                // "/exit ", then press Enter to run it. Tab moves the cursor beyond
...
mod tests {
    ...
    fn footer_hint_row_is_separated_from_composer() {
        ...
        let composer = ChatComposer::new(true, sender, false, false);
        ...
        F: FnOnce(&mut ChatComposer),
    {
        ...
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let sender = AppEventSender::new(tx);
        let mut composer = ChatComposer::new(true, sender, enhanced_keys_supported, false);
        setup(&mut composer);
    ...
    fn esc_hint_stays_hidden_with_draft_content() {
        ...
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let sender = AppEventSender::new(tx);
        let mut composer = ChatComposer::new(true, sender, true, false);
    ...
    #[test]
    fn slash_new_dispatches_command_and_does_not_submit_literal_text() {
        use crossterm::event::KeyCode;
        use crossterm::event::KeyEvent;
        use crossterm::event::KeyModifiers;
        ...
        type_chars_humanlike(&mut composer, &['/', 'n', 'e', 'w']);
        ...
        match result {
            ...
            InputResult::None => panic!("expected Command result for '/new'"),
        }
    ...
    fn slash_tab_then_enter_dispatches_builtin_command() {
        ...
        type_chars_humanlike(&mut composer, &['/', 'e', 'x']);
        let (_res, _redraw) =
            composer.handle_key_event(KeyEvent::new(KeyCode::Tab, KeyModifiers::NONE));
        assert_eq!(composer.textarea.text(), "/exit ");

        // Press Enter: should dispatch the command, not submit literal text.
        let (result, _needs_redraw) =
            composer.handle_key_event(KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE));
        match result {
            InputResult::Command(cmd) => assert_eq!(cmd.command(), "exit"),
            InputResult::Submitted(text) => {
                panic!("expected command dispatch after Tab completion, got literal submit: {text}")
            }
            InputResult::None => panic!("expected Command result for '/exit'"),
        }
    ...
    fn pasting_filepath_attaches_image() {
        ...
        let tmp_path: PathBuf = tmp.path().join("aish_tui_test_paste_image.png");
        ...
        assert!(
            composer
                .textarea
                .text()
                .starts_with("[aish_tui_test_paste_image.png 3x2] ")
        );
