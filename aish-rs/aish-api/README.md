...
Typed clients for Codex/OpenAI APIs built on top of the generic transport in `aish-client`.
...
- Serves as the wire-level layer consumed by `aish-core`; higher layers handle auth refresh and business logic.
...
- **Prompted endpoints (Chat + Responses)**
  - Input: a single `Prompt` plus endpoint-specific options.
    - `Prompt` (re-exported as `aish_api::Prompt`) carries:
...
- **Compaction endpoint**
  - Input: `CompactionInput<'a>` (re-exported as `aish_api::CompactionInput`):
    - `model: &str`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `aish-api` and `aish-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
