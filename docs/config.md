...
Aish configuration gives you fine-grained control over the model, execution environment, and integrations available to the CLI. Use this guide alongside the workflows in [`aish exec`](./exec.md), the guardrails in [Sandbox & approvals](./sandbox.md), and project guidance from [AGENTS.md discovery](./agents_md.md).
...
Aish supports several mechanisms for setting config values:
...
- A generic `-c`/`--config` flag that takes a `key=value` pair, such as `--config model="o3"`.
  ...
  - For consistency with `config.toml`, values are a string in TOML format rather than JSON format, so use `key='{a = 1, b = 2}'` rather than `key='{"a": 1, "b": 2}'`.
    - The quotes around the value are necessary, as without them your shell would split the config argument on spaces, resulting in `aish` receiving `-c key={a` with (invalid) additional arguments `=`, `1,`, `b`, `=`, `2}`.
...
- The `$AISH_HOME/config.toml` configuration file where the `AISH_HOME` environment value defaults to `~/.aish`. (Note `AISH_HOME` will also be where logs and other Aish-related information are stored.)

Both the `--config` flag and the `config.toml` file support the following options:

## Feature flags

Optional and experimental capabilities are toggled via the `[features]` table in `$AISH_HOME/config.toml`. If you see a deprecation notice mentioning a legacy key (for example `experimental_use_exec_command_tool`), move the setting into `[features]` or pass `--enable <feature>`.
...
### Undo snapshots (Ghost snapshots)

Ghost snapshots are lightweight repository snapshots captured每回合，用于支持撤销（Undo）。它们仅作用于代码仓库内容，不包含“Shell 环境快照”（该功能已移除）。相关配置项位于 `ghost_snapshot.*`，例如忽略超大未跟踪文件或目录，以及禁用快照警告；详见配置参考表与注释。

## Model selection

### model

The model that Aish should use.
...
# Name of the provider that will be displayed in the Aish UI.
name = "OpenAI using Chat Completions"
# The path `/chat/completions` will be amended to this URL to make the POST
# request for the chat completions.
base_url = "https://api.openai.com/v1"
# If `env_key` is set, identifies an environment variable that must be set when
# using Aish with this provider. The value of the environment variable must be
...
Note this makes it possible to use Aish CLI with non-OpenAI models, so long as they use a wire API that is compatible with the OpenAI chat completions API. For example, you could define the following provider to use Aish CLI with Ollama running locally:
...
Export your key before launching Aish: `export AZURE_OPENAI_API_KEY=…`
...
How many times Aish will retry a failed HTTP request to the model provider. Defaults to `4`.

##### stream_max_retries

Number of times Aish will attempt to reconnect when a streaming response is interrupted. Defaults to `5`.

##### stream_idle_timeout_ms

How long Aish will wait for activity on a streaming response before treating the connection as lost. Defaults to `300_000` (5 minutes).
...
When set, Aish includes a `text` object in the request payload with the configured verbosity, for example: `"text": { "verbosity": "low" }`.
...
In general, Aish knows the context window for the most common OpenAI models, but if you are using a new model with an old version of the Aish CLI, then you can use `model_context_window` to tell Aish what value to use to determine how much context is left during a conversation.

### oss_provider

Specifies the default OSS provider to use when running Aish. This is used when the `--oss` flag is provided without a specific provider.
...
Determines when the user should be prompted to approve whether Aish can execute a command:

```toml
# Aish has hardcoded logic that defines a set of "trusted" commands.
# Setting the approval_policy to `untrusted` means that Aish will prompt the
...
```toml
# If the command fails when run in the sandbox, Aish asks for permission to
...
```toml
# User is never prompted: if the command fails, Aish will automatically try
...
Aish executes model-generated shell commands inside an OS-level sandbox.
...
A more relaxed policy is `workspace-write`. When specified, the current working directory for the Aish task will be writable (as well as `$TMPDIR` on macOS). Note that the CLI defaults to using the directory where it was spawned as `cwd`, though this can be overridden using `--cwd/-C`.

On macOS (and soon Linux), all writable roots (including `cwd`) that contain a `.git/` or `.aish/` folder _as an immediate child_ will configure those folders to be read-only while the rest of the root stays writable. This means that commands like `git commit` will fail, by default (as it entails writing to `.git/`), and will require Aish to ask for permission.
...
# By default, the cwd for the Aish session will be writable as well as $TMPDIR
...
This is reasonable to use if Aish is running in an environment that provides its own sandboxing (such as a Docker container) such that further sandboxing is unnecessary.

Though using this option may also be necessary if you try to use Aish in environments where its native sandboxing mechanisms are unsupported, such as older Linux kernels or on Windows.
...
```toml
[tools]
web_search = true   # allow Aish to issue first-party web searches without prompting you (deprecated)
view_image = false  # disable image uploads (they're enabled by default)
```

The `view_image` toggle is useful when you want to include screenshots or diagrams from your repo without pasting them manually. Aish still respects sandboxing: it can only attach files inside the workspace roots you allow.

### approval_presets

Aish provides three main Approval Presets:

- Read Only: Aish can read files and answer questions; edits, running commands, and network access require approval.
- Auto: Aish can read files, make edits, and run commands in the workspace without approval; asks for approval outside the workspace or for network access.
- Full Access: Full disk and network access without prompts; extremely risky.

You can further customize how Aish runs at the command line using the `--ask-for-approval` and `--sandbox` options.

> See also [Sandbox & approvals](./sandbox.md) for in-depth examples and platform-specific behaviour.

### shell_environment_policy

Aish spawns subprocesses (e.g. when executing a `local_shell` tool-call suggested by the assistant). By default it now passes **your full environment** to those subprocesses. You can tune this behavior via the **`shell_environment_policy`** block in `config.toml`:
...
| `ignore_default_excludes` | boolean              | `true`  | When `false`, Aish removes any var whose **name** contains `KEY`, `SECRET`, or `TOKEN` (case-insensitive) before other rules run.              |
...
Currently, `AISH_SANDBOX_NETWORK_DISABLED=1` is also added to the environment, assuming network is disabled. This is not configurable.

## Project root detection

Aish discovers `.aish/` project layers by walking up from the working directory until it hits a project marker. By default it looks for `.git`. You can override the marker list in user/system/MDM config:
...
You can configure Aish to use [MCP servers](https://modelcontextprotocol.io/about) to give Aish access to external applications, resources, or services.
...
[Streamable HTTP servers](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) enable Aish to talk to resources that are accessed via a http url (either on localhost or another domain).
...
Streamable HTTP connections always use the Rust MCP client under the hood. Run `aish mcp login <server-name>` to authenticate for servers supporting OAuth.
...
When both `enabled_tools` and `disabled_tools` are specified, Aish first restricts the server to the allow-list and then removes any tools that appear in the deny-list.
...
aish mcp add docs -- docs-server --port 4000
...
aish mcp get docs
aish mcp get docs --json

# Remove a server
aish mcp remove docs

# Log in to a streamable HTTP server that supports oauth
aish mcp login SERVER_NAME

# Log out from a streamable HTTP server that supports oauth
aish mcp logout SERVER_NAME
```

### Examples of useful MCPs

There is an ever growing list of useful MCP servers that can be helpful while you are working with Aish.
...
Aish can emit [OpenTelemetry](https://opentelemetry.io/) **log events** that
...
Aish tags every exported event with `service.name = $ORIGINATOR` (the same
value sent in the `originator` header, `aish_cli_rs` by default), the CLI
version, and an `env` attribute so downstream collectors can distinguish
dev/staging/prod traffic. Only telemetry produced inside the `aish_otel`
...
With OTEL enabled Aish emits the following event types (in addition to the
metadata above):
...
or enable mutual TLS. Relative paths are resolved against `~/.aish/`:
...
client-certificate = "/etc/aish/certs/client.pem"
client-private-key = "/etc/aish/certs/client-key.pem"
```
...
If you build Aish from source the OTEL crate is still behind an `otel` feature
...
Specify a program that will be executed to get notified about events generated by Aish. Note that the program will receive the notification argument as a string of JSON, e.g.:
...
`"thread-id"` contains a string that identifies the Aish session that produced the notification; you can use it to correlate multiple turns that belong to the same task.
    ...
    match notification_type := notification.get("type"):
        case "agent-turn-complete":
            ...
            if assistant_message:
                title = f"Aish: {assistant_message}"
            else:
                title = "Aish: Turn Complete!"
...
To have Aish use this script for notifications, you would configure it via `notify` in `~/.aish/config.toml` using the appropriate path to `notify.py` on your computer:

```toml
notify = ["python3", "/Users/mbolin/.aish/notify.py"]
```
...
4. the default value that comes with Aish CLI (i.e., Aish CLI defaults to `gpt-5.1-codex-max`)

### history

By default, Aish CLI records messages sent to the model in `$AISH_HOME/history.jsonl`. Note that on UNIX, the file permissions are set to `o600`, so it should only be readable and writable by the owner.
...
count. When the file grows beyond the limit, Aish removes the oldest entries,
...
Currently, `"vscode"` is the default, though Aish does not verify VS Code is installed. As such, `file_opener` may default to `"none"` or something else in the future.

### project_doc_max_bytes

Maximum number of bytes to read from an `AISH.md` file to include in the instructions sent with the first turn of a session. Defaults to 32 KiB.

### project_doc_fallback_filenames

Ordered list of additional filenames to look for when `AISH.md` is missing at a given directory level. The CLI always checks `AISH.md` first; the configured fallbacks are tried in the order provided. This lets monorepos that already use alternate instruction files (for example, `CLAUDE.md`) work out of the box while you migrate to `AISH.md` over time.

```toml
project_doc_fallback_filenames = ["CLAUDE.md", ".exampleagentrules.md"]
```

We recommend migrating instructions to AISH.md; other filenames may reduce model performance.

> See also [AISH.md discovery](./agents_md.md) for how Aish locates these files during a session.
...
> [!NOTE]
> Aish emits desktop notifications using terminal escape codes. Not all terminals support these (notably, macOS Terminal.app and VS Code's terminal do not support custom notifications. iTerm2, Ghostty and WezTerm do support these notifications).
...
To force users on a given machine to use a specific login method or workspace, use a combination of managed configurations as well as either or both of the following fields:
...
# flow and the id will be validated during the oauth callback as well as every time Aish starts.
forced_chatgpt_workspace_id = "00000000-0000-0000-0000-000000000000"
```

If the active credentials don't match the config, the user will be logged out and Aish will exit.
...
| `mcp_servers.<id>.enabled`                       | boolean                                                           | When false, Aish skips starting the server (default: true).                                                                    |
...
| `project_doc_max_bytes`                          | number                                                            | Max bytes to read from `AISH.md`.                                                                                             |
...
| `tools.view_image`                               | boolean                                                           | Enable or disable the `view_image` tool so Aish can attach local image files from the workspace (default: true).               |
| `forced_login_method`                            | `chatgpt` \| `api`                                                | Only allow Aish to be used with ChatGPT or API keys.                                                                           |
| `forced_chatgpt_workspace_id`                    | string (uuid)                                                     | Only allow Aish to be used with the specified ChatGPT workspace.                                                               |
