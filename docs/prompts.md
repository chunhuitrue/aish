...
- Location: store prompts in `$AISH_HOME/prompts/` (defaults to `~/.aish/prompts/`). Set `AISH_HOME` if you want to use a different folder.
...
- Naming: The filename (without `.md`) becomes the prompt name. A file called `code-review.md` registers the prompt `code-review`.
...
**File**: `~/.aish/prompts/code-review.md`
...
```
/prompts:code-review FILE=src/auth.js FOCUS="error handling"
```
