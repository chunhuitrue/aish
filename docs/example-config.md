...
Use this example configuration as a starting point. For an explanation of each field and additional context, see [Configuration](./config.md). Copy the snippet below to `~/.aish/config.toml` and adjust values as needed.

```toml
# Aish example configuration (config.toml)
#
# This file lists all keys Aish reads from config.toml, their default values,
...
# Primary model used by Aish. Default: "gpt-5.1-codex-max" on all platforms.
...
# Optional manual model metadata. When unset, Aish auto-detects from model.
...
# In-product notices (mostly set automatically by Aish).
[notice]
...
# Force login mechanism when Aish would normally auto-select. Default: unset.
